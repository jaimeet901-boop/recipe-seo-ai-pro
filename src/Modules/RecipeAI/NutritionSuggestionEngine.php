...
/**
 * Nutrition suggestion engine (Phase 5.2).
 *
...
/**
 * Class NutritionSuggestionEngine
 */
final class NutritionSuggestionEngine {
	 ...
	 */
	public function run( array $recipe ) {
		...
		$prompt   = $this->prompts->build( 'nutrition', '', $recipe );
		$provider = $this->providers->get( 'openai_compatible' );
		$result   = $provider->complete( $prompt, array( 'max_tokens' => 900 ) );
		if ( is_wp_error( $result ) ) {
			$this->logger->error( 'recipe_ai.nutrition_failed', array( 'message' => $result->get_error_message() ) );
			return $result;
		}
		$data = $this->decoder->decode( (string) $result );
		if ( ! is_array( $data ) ) {
			return new \WP_Error( 'rsaip_recipe_ai_parse', 'Could not parse nutrition suggestions.' );
		}
		return $data;
	}
}
